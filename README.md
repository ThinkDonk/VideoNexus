...
**Q：WVP/门户里出现设备上并不存在的通道（如 AudioOut1）**
多为**平台侧历史残留**而非设备上报：触发一次目录同步即可清除（WVP 设备页「同步目录」，或门户管理端的「从 WVP 同步」）。判断方法：同步后该通道若消失，说明设备当前并不上报它。
若同步后仍存在，则确实是设备上报的虚拟通道（国标编码第 11–13 位为 `137` 表示音频输出、`134`/`135` 表示报警输入/输出），常见于 NVR 为支持语音对讲而上报的音频通道，海康部分固件的 GB28181 页面不提供取消入口。这类通道**不影响使用**：门户的摄像树只显示"仍存在于设备目录且已被授权"的通道；若确实存在残留授权，授权管理页会以「已失效」标记显示，保存后自动收回。
